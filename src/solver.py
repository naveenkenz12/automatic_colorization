...
from data_procedd import DataSet
import time
...
class Solver(object):
  ...
  def construct_graph(self, scope):
    ...
    self.final_u,final_v = self.net.create_net(self.data_l)
    new_loss = self.net.loss(scope, self.conv8_313, self.prior_boost_nongray, self.gt_ab_313)
